...
// Rotate right generator.
#define ROTGEN(bits)                                                                                                   \
    static inline uint##bits##_t roru##bits(uint##bits##_t a, int b) {                                                 \
        b %= bits;                                                                                                     \
        return (a >> b) | (a << (bits - b));                                                                           \
    }                                                                                                                  \
    static inline int##bits##_t rori##bits(int##bits##_t a, int b) {                                                   \
        return roru##bits(a, b);                                                                                       \
    }                                                                                                                  \
    static inline uint##bits##_t rolu##bits(uint##bits##_t a, int b) {                                                 \
        b %= bits;                                                                                                     \
        return (a << b) | (a >> (bits - b));                                                                           \
    }                                                                                                                  \
    static inline int##bits##_t roli##bits(int##bits##_t a, int b) {                                                   \
        return rolu##bits(a, b);                                                                                       \
    }
ROTGEN(8)
ROTGEN(16)
ROTGEN(32)
...
#ifndef KILOBOOTLOADER
// CRC32 magic table.
static uint32_t const crc_tab[256] = {
...
crc32_t _crc32_byte(crc32_t crc, uint8_t val) {
    return (crc32_t){
        (crc >> 8) ^ crc_tab[(crc ^ val) & 255],
    };
}
#else
// Add a byte to a CRC32 checksum.
...
crc32_t _crc32_update(crc32_t crc, void const *mem, size_t len) {
    uint8_t const *ptr = mem;
    for (size_t i = 0; i < len; i++) {
        crc = _crc32_byte(crc, ptr[i]);
    }
    return crc;
}
