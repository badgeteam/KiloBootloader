...
typedef volatile struct {
    // The current status of SPI0 master FSM: spi0_mst_st. 0: idle state, 1:SPI0_GRANT, 2: program/erase suspend state,
    // 3: SPI0 read data state, 4: wait cache/EDMA sent data is stored in SPI0 TX FIFO, 5: SPI0 write data state.
    uint32_t mst_st     : 4;
    // The current status of SPI0 slave FSM: mspi_st. 0: idle state, 1: preparation state, 2: send command state, 3:
    // send address state, 4: wait state, 5: read data state, 6:write data state, 7: done state, 8: read data end state.
    uint32_t slv_st     : 4;
    uint32_t            : 9;
    // In user mode, it is set to indicate that program/erase operation will be triggered. The bit is combined with
    // spi_mem_usr bit. The bit will be cleared once the operation done.1: enable 0: disable.
    uint32_t flash_pe   : 1;
    // SPI0 USR_CMD start bit, only used when SPI_MEM_AXI_REQ_EN is cleared.  An operation will be triggered when the
    ...
    // This bit combined with reg_resandres bit releases Flash from the power-down state or high performance mode and
    // obtains the devices ID. The bit will be cleared once the operation done.1: enable 0: disable.
    uint32_t flash_res  : 1;
    // Drive Flash into power down.  An operation will be triggered when the bit is set. The bit will be cleared once
    // the operation done.1: enable 0: disable.
    uint32_t flash_dp   : 1;
    // Chip erase enable. Chip erase operation will be triggered when the bit is set. The bit will be cleared once the
    // operation done.1: enable 0: disable.
    uint32_t flash_ce   : 1;
    // Block erase enable(32KB) .  Block erase operation will be triggered when the bit is set. The bit will be cleared
    // once the operation done.1: enable 0: disable.
    uint32_t flash_be   : 1;
    // Sector erase enable(4KB). Sector erase operation will be triggered when the bitis set. The bit will be cleared
    // once the operation done.1: enable 0: disable.
    uint32_t flash_se   : 1;
    // Page program enable(1 byte ~256 bytes data to be programmed). Page program operation  will be triggered when the
    // bit is set. The bit will be cleared once the operation done .1: enable 0: disable.
    uint32_t flash_pp   : 1;
    // Write status register enable.   Write status operation  will be triggered when the bit is set. The bit will be
    // cleared once the operation done.1: enable 0: disable.
    uint32_t flash_wrsr : 1;
    // Read status register-1.  Read status operation will be triggered when the bit is set. The bit will be cleared
    // once the operation done.1: enable 0: disable.
    uint32_t flash_rdsr : 1;
    // Read JEDEC ID . Read ID command will be sent when the bit is set. The bit will be cleared once the operation
    // done. 1: enable 0: disable.
    uint32_t flash_rdid : 1;
    // Write flash disable. Write disable command will be sent when the bit is set. The bit will be cleared once the
    // operation done. 1: enable 0: disable.
    uint32_t flash_wrdi : 1;
    // Write flash enable.  Write enable command will be sent when the bit is set. Thebit will be cleared once the
    // operation done. 1: enable 0: disable.
    uint32_t flash_wren : 1;
    // Read flash enable. Read flash operation will be triggered when the bit is set. The bit will be cleared once the
    // operation done. 1: enable 0: disable.
    uint32_t flash_read : 1;
} spimem_cmd_t;

typedef volatile struct {
    // In the dummy phase of an MSPI write data transfer when accesses to flash, the level of SPI_DQS is output by the
    // MSPI controller.
    uint32_t wdummy_dqs_always_out : 1;
    // In the dummy phase of an MSPI write data transfer when accesses to flash, the level of SPI_IO[7:0] is output by
    // the MSPI controller.
    uint32_t wdummy_always_out     : 1;
    // In an MSPI read data transfer when accesses to flash, the level of SPI_IO[7:0] is output by the MSPI controller
    // in the first half part of dummy phase. It is used to mask invalid SPI_DQS in the half part of dummy phase.
    uint32_t fdummy_rin            : 1;
    // In an MSPI write data transfer when accesses to flash, the level of SPI_IO[7:0]is output by the MSPI controller
    // in the second half part of dummy phase. It is used to pre-drive flash.
    ...
    uint32_t                       : 1;
    // This bit enable the bits: SPI_MEM_FREAD_QIO, SPI_MEM_FREAD_DIO, SPI_MEM_FREAD_QOUT and SPI_MEM_FREAD_DOUT. 1:
    // enable 0: disable.
    uint32_t fastrd_mode           : 1;
    // In the read operations, read-data phase apply 2 signals. 1: enable 0: disable.
    uint32_t fread_dual            : 1;
    // The Device ID is read out to SPI_MEM_RD_STATUS register,  this bit combine withspi_mem_flash_res bit. 1: enable
    // 0: disable.
...
typedef volatile struct {
    // SPI clock mode bits. 0: SPI clock is off when CS inactive 1: SPI clock is delayed one cycle after CS inactive 2:
    // SPI clock is delayed two cycles after CS inactive 3: SPI clock is alwasy on.
    uint32_t clk_mode              : 2;
    // After RES/DP/HPM command is sent, SPI1 waits (SPI_MEM_CS_HOLD_DELAY_RES[9:0] * 512) SPI_CLK cycles.
    uint32_t cs_hold_dly_res       : 10;
    uint32_t                       : 9;
    // 1: MSPI supports ARSIZE 0~3. When ARSIZE =0~2, MSPI read address is 4*n and reply the real AXI read data back. 0:
    // When ARSIZE 0~1, MSPI reply SLV_ERR.
    ...
    // 1: Reply AXI read data to AXI bus when one AXI read beat data is available. 0: Reply AXI read data to AXI bus
    // when all the read data is available.
    uint32_t axi_rdata_back_fast   : 1;
    // 1: RRESP is SLV_ERR when there is a ECC error in AXI read data. 0: RRESP is OKAY when there is a ECC error in AXI
    // read data. The ECC error information is recorded in SPI_MEM_ECC_ERR_ADDR_REG.
    ...
    // When SPI_MEM_DUAL_RAM_EN is 0 and SPI_MEM_RAM0_EN is 1, only EXT_RAM0 will be accessed. When SPI_MEM_DUAL_RAM_EN
    // is 0 and SPI_MEM_RAM0_EN is 0, only EXT_RAM1 will be accessed. When SPI_MEM_DUAL_RAM_EN is 1,  EXT_RAM0 and
    // EXT_RAM1 will be accessed at the same time.
    uint32_t ram0_en               : 1;
    // Set this bit to enable DUAL-RAM mode, EXT_RAM0 and EXT_RAM1 will be accessed atthe same time.
    uint32_t dual_ram_en           : 1;
    // Set this bit to write data faster, do not wait write data has been stored in tx_bus_fifo_l2. It will wait
    // 4*T_clk_ctrl to insure the write data has been storedin  tx_bus_fifo_l2.
    uint32_t fast_write_en         : 1;
    // The synchronous reset signal for SPI0 RX AFIFO and all the AES_MSPI SYNC FIFO to receive signals from AXI.  Set
    // this bit to reset these FIFO.
    uint32_t rxfifo_rst            : 1;
    // The synchronous reset signal for SPI0 TX AFIFO and all the AES_MSPI SYNC FIFO to send signals to AXI. Set this
    // bit to reset these FIFO.
...
typedef volatile struct {
    ...
    uint32_t                      : 9;
    // Set this bit to enable SPI0 split one AXI read flash transfer into two SPI transfers when one transfer will cross
    // flash or EXT_RAM page corner, valid no matterwhether there is an ECC region or not.
    uint32_t split_trans_en       : 1;
    // These bits are used to set the minimum CS high time tSHSL between SPI burst transfer when accesses to flash.
    // tSHSL is (SPI_MEM_CS_HOLD_DELAY[5:0] + 1) MSPI core clock cycles.
...
typedef volatile struct {
    // SPI_CS0 pin enable, 1: disable SPI_CS0, 0: SPI_CS0 pin is active to select SPI device, such as flash, external
    // RAM and so on.
    uint32_t cs0_dis        : 1;
    // SPI_CS1 pin enable, 1: disable SPI_CS1, 0: SPI_CS1 pin is active to select SPI device, such as flash, external
    // RAM and so on.
    uint32_t cs1_dis        : 1;
    uint32_t                : 5;
...
typedef volatile struct {
    ...
    uint32_t cache_flash_usr_cmd    : 1;
    // For SPI0 flash, din phase apply 2 signals. 1: enable 0: disable. The bit is thesame with spi_mem_fread_dio.
    ...
    // Set this bit to close AXI read/write transfer to MSPI, which means that only SLV_ERR will be replied to
    // BRESP/RRESP.
    uint32_t close_axi_inf_en       : 1;
} spimem_cache_fctrl_t;

typedef volatile struct {
    // For SPI0, In the external RAM mode, cache read flash with 4 bytes command, 1: enable, 0:disable.
    uint32_t cache_usr_saddr_4byte : 1;
    ...
    // For SPI0, In the external RAM mode, it is the length in bits of read dummy phase. The register value shall be
    // (bit_num-1).
    uint32_t sram_rdummy_cyclelen  : 6;
    uint32_t                       : 2;
    // For SPI0, In the external RAM mode, it is the length in bits of address phase. The register value shall be
    // (bit_num-1).
    ...
    // reserved
    uint32_t sram_oct              : 1;
    // For SPI0, In the external RAM mode, it is the length in bits of write dummy phase. The register value shall be
    // (bit_num-1).
    uint32_t sram_wdummy_cyclelen  : 6;
    uint32_t                       : 4;
} spimem_cache_sctrl_t;

typedef volatile struct {
    // SPI clock mode bits. 0: SPI clock is off when CS inactive 1: SPI clock is delayed one cycle after CS inactive 2:
    // SPI clock is delayed two cycles after CS inactive 3: SPI clock is always on.
    uint32_t sclk_mode                  : 2;
    // Mode bits in the external RAM fast read mode  it is combined with spi_mem_fastrd_mode bit.
    uint32_t swb_mode                   : 8;
    // For SPI0 external RAM , din phase apply 2 signals. 1: enable 0: disable. The bit is the same with
    // spi_mem_usr_sram_dio.
    uint32_t sdin_dual                  : 1;
    // For SPI0 external RAM , dout phase apply 2 signals. 1: enable 0: disable. The bit is the same with
    // spi_mem_usr_sram_dio.
    uint32_t sdout_dual                 : 1;
    // For SPI0 external RAM , address phase apply 2 signals. 1: enable 0: disable. The bit is the same with
    // spi_mem_usr_sram_dio.
    uint32_t saddr_dual                 : 1;
    uint32_t                            : 1;
    // For SPI0 external RAM , din phase apply 4 signals. 1: enable 0: disable. The bit is the same with
    // spi_mem_usr_sram_qio.
    uint32_t sdin_quad                  : 1;
    // For SPI0 external RAM , dout phase apply 4 signals. 1: enable 0: disable. The bit is the same with
    // spi_mem_usr_sram_qio.
    uint32_t sdout_quad                 : 1;
    // For SPI0 external RAM , address phase apply 4 signals. 1: enable 0: disable. The bit is the same with
    // spi_mem_usr_sram_qio.
    uint32_t saddr_quad                 : 1;
    // For SPI0 external RAM , cmd phase apply 4 signals. 1: enable 0: disable. The bit is the same with
    // spi_mem_usr_sram_qio.
    ...
    uint32_t scmd_oct                   : 1;
    // In the dummy phase of a MSPI read data transfer when accesses to external RAM, the signal level of SPI bus is
    // output by the MSPI controller.
    uint32_t sdummy_rin                 : 1;
    // In the dummy phase of a MSPI write data transfer when accesses to external RAM,the signal level of SPI bus is
    // output by the MSPI controller.
    uint32_t sdummy_wout                : 1;
    // In the dummy phase of an MSPI write data transfer when accesses to external RAM, the level of SPI_DQS is output
    // by the MSPI controller.
    uint32_t smem_wdummy_dqs_always_out : 1;
    // In the dummy phase of an MSPI write data transfer when accesses to external RAM, the level of SPI_IO[7:0] is
    // output by the MSPI controller.
...
typedef volatile struct {
    ...
    uint32_t cache_sram_usr_rd_cmd_value  : 16;
    uint32_t                              : 12;
    // For SPI0,When cache mode is enable it is the length in bits of command phase for sram. The register value shall
    // be (bit_num-1).
    uint32_t cache_sram_usr_rd_cmd_bitlen : 4;
} spimem_sram_drd_cmd_t;

typedef volatile struct {
    // For SPI0,When cache mode is enable it is the write command value of command phase for sram.
    uint32_t cache_sram_usr_wr_cmd_value  : 16;
    uint32_t                              : 12;
    // For SPI0,When cache mode is enable it is the in bits of command phase  for sram. The register value shall be
    // (bit_num-1).
    uint32_t cache_sram_usr_wr_cmd_bitlen : 4;
} spimem_sram_dwr_cmd_t;

typedef volatile struct {
    // For SPI0 external RAM  interface, it must be equal to spi_mem_clkcnt_N.
    uint32_t sclkcnt_l       : 8;
    // For SPI0 external RAM  interface, it must be floor((spi_mem_clkcnt_N+1)/2-1).
    uint32_t sclkcnt_h       : 8;
    // For SPI0 external RAM  interface, it is the divider of spi_mem_clk. So spi_mem_clk frequency is
    // system/(spi_mem_clkcnt_N+1)
    uint32_t sclkcnt_n       : 8;
    uint32_t                 : 7;
    // For SPI0 external RAM  interface, 1: spi_mem_clk is eqaul to system 0: spi_mem_clk is divided from system clock.
    uint32_t sclk_equ_sysclk : 1;
} spimem_sram_clk_t;

typedef volatile struct {
    // 1: The hardware will wait idle after SE/PP/WRSR automatically, and hardware auto Suspend/Resume can be enabled.
    // 0: The functions of hardware wait idle and autoSuspend/Resume are not supported.
    uint32_t waiti_en             : 1;
    // The dummy phase enable when wait flash idle (RDSR)
    uint32_t waiti_dummy          : 1;
    // 1: Output  address 0 in RDSR or read SUS command transfer. 0: Do not send out address in RDSR or read SUS command
    // transfer.
    uint32_t waiti_addr_en        : 1;
    // When SPI_MEM_WAITI_ADDR_EN is set, the  cycle length of sent out address is (SPI_MEM_WAITI_ADDR_CYCLELEN[1:0] +
    // 1) SPI  bus clock cycles. It is not active whenSPI_MEM_WAITI_ADDR_EN is cleared.
...
} spimem_flash_waiti_ctrl_t;

typedef volatile struct {
    // program erase resume bit, program erase suspend operation will be triggered when the bit is set. The bit will be
    // cleared once the operation done.1: enable 0: disable.
    uint32_t flash_per         : 1;
    // program erase suspend bit, program erase suspend operation will be triggered when the bit is set. The bit will be
    // cleared once the operation done.1: enable 0: disable.
    uint32_t flash_pes         : 1;
    // 1: SPI1 waits (SPI_MEM_CS_HOLD_DELAY_RES[9:0] * 4 or *128) SPI_CLK cycles afterprogram erase resume command is
    // sent. 0: SPI1 does not wait after program eraseresume command is sent.
    uint32_t flash_per_wait_en : 1;
    // 1: SPI1 waits (SPI_MEM_CS_HOLD_DELAY_RES[9:0] * 4 or *128) SPI_CLK cycles afterprogram erase suspend command is
    // sent. 0: SPI1 does not wait after program erase suspend command is sent.
    uint32_t flash_pes_wait_en : 1;
    // Set this bit to enable PES end triggers PER transfer option. If this bit is 0, application should send PER after
    // PES is done.
    uint32_t pes_per_en        : 1;
    // Set this bit to enable Auto-suspending function.
    uint32_t flash_pes_en      : 1;
    // The mask value when check SUS/SUS1/SUS2 status bit. If the read status value isstatus_in[15:0](only
    // status_in[7:0] is valid when only one byte of data is readout, status_in[15:0] is valid when two bytes of data
    // are read out), SUS/SUS1/SUS2 = status_in[15:0]^ SPI_MEM_PESR_END_MSK[15:0].
    uint32_t pesr_end_msk      : 16;
    // 1: Read two bytes when check flash SUS/SUS1/SUS2 status bit. 0:  Read one byte when check flash SUS/SUS1/SUS2
    // status bit
    uint32_t fmem_rd_sus_2b    : 1;
    // 1: Both WIP and SUS/SUS1/SUS2 bits should be checked to insure the resume status of flash. 0: Only need to check
    // WIP is 0.
    uint32_t per_end_en        : 1;
    // 1: Both WIP and SUS/SUS1/SUS2 bits should be checked to insure the suspend status of flash. 0: Only need to check
    // WIP is 0.
    uint32_t pes_end_en        : 1;
    // When SPI1 checks SUS/SUS1/SUS2 bits fail for SPI_MEM_SUS_TIMEOUT_CNT[6:0] times, it will be treated as check
    // pass.
    uint32_t sus_timeout_cnt   : 7;
} spimem_flash_sus_ctrl_t;

typedef volatile struct {
    // Program/Erase suspend command.
    uint32_t flash_pes_command : 16;
    // Flash SUS/SUS1/SUS2 status bit read command. The command should be sent when SUS/SUS1/SUS2 bit should be checked
    // to insure the suspend or resume status of flash.
    uint32_t wait_pesr_command : 16;
} spimem_flash_sus_cmd_t;

typedef volatile struct {
    // The status of flash suspend, only used in SPI1.
    uint32_t flash_sus         : 1;
    // 1: SPI1 sends out SPI_MEM_WAIT_PESR_COMMAND[15:0] to check SUS/SUS1/SUS2 bit. 0: SPI1 sends out
    // SPI_MEM_WAIT_PESR_COMMAND[7:0] to check SUS/SUS1/SUS2 bit.
    uint32_t wait_pesr_cmd_2b  : 1;
    // 1: SPI1 waits (SPI_MEM_CS_HOLD_DELAY_RES[9:0] * 128) SPI_CLK cycles after HPM command is sent. 0: SPI1 waits
    // (SPI_MEM_CS_HOLD_DELAY_RES[9:0] * 4) SPI_CLK cycles after HPM command is sent.
    uint32_t flash_hpm_dly_128 : 1;
    // 1: SPI1 waits (SPI_MEM_CS_HOLD_DELAY_RES[9:0] * 128) SPI_CLK cycles after RES command is sent. 0: SPI1 waits
    // (SPI_MEM_CS_HOLD_DELAY_RES[9:0] * 4) SPI_CLK cycles after RES command is sent.
    uint32_t flash_res_dly_128 : 1;
    // 1: SPI1 waits (SPI_MEM_CS_HOLD_DELAY_RES[9:0] * 128) SPI_CLK cycles after DP command is sent. 0: SPI1 waits
    // (SPI_MEM_CS_HOLD_DELAY_RES[9:0] * 4) SPI_CLK cyclesafter DP command is sent.
    uint32_t flash_dp_dly_128  : 1;
    // Valid when SPI_MEM_FLASH_PER_WAIT_EN is 1. 1: SPI1 waits (SPI_MEM_CS_HOLD_DELAY_RES[9:0] * 128) SPI_CLK cycles
    // after PER command is sent. 0: SPI1 waits (SPI_MEM_CS_HOLD_DELAY_RES[9:0] * 4) SPI_CLK cycles after PER command is
    // sent.
    uint32_t flash_per_dly_128 : 1;
    // Valid when SPI_MEM_FLASH_PES_WAIT_EN is 1. 1: SPI1 waits (SPI_MEM_CS_HOLD_DELAY_RES[9:0] * 128) SPI_CLK cycles
    // after PES command is sent. 0: SPI1 waits (SPI_MEM_CS_HOLD_DELAY_RES[9:0] * 4) SPI_CLK cycles after PES command is
    // sent.
    uint32_t flash_pes_dly_128 : 1;
    // 1: Enable SPI0 lock SPI0/1 arbiter option. 0: Disable it.
    uint32_t spi0_lock_en      : 1;
    uint32_t                   : 7;
    // 1: The bit length of Program/Erase Suspend/Resume command is 16. 0: The bit length of Program/Erase
    // Suspend/Resume command is 8.
...
typedef volatile struct {
    // The raw bit for SPI_MEM_PER_END_INT interrupt. 1: Triggered when Auto Resume command (0x7A) is sent and flash is
    // resumed successfully. 0: Others.
    uint32_t per_end_int_raw          : 1;
    // The raw bit for SPI_MEM_PES_END_INT interrupt.1: Triggered when Auto Suspend command (0x75) is sent and flash is
    // suspended successfully. 0: Others.
    uint32_t pes_end_int_raw          : 1;
    // The raw bit for SPI_MEM_WPE_END_INT interrupt. 1: Triggered when WRSR/PP/SE/BE/CE is sent and flash is already
    // idle. 0: Others.
    uint32_t wpe_end_int_raw          : 1;
    // The raw bit for SPI_MEM_SLV_ST_END_INT interrupt. 1: Triggered when spi0_slv_stis changed from non idle state to
    // idle state. It means that SPI_CS raises high.0: Others
    uint32_t slv_st_end_int_raw       : 1;
    // The raw bit for SPI_MEM_MST_ST_END_INT interrupt. 1: Triggered when spi0_mst_stis changed from non idle state to
    // idle state. 0: Others.
    ...
    // is cleared, this bit is triggered when the error times of SPI0/1 ECC read flash are equal or bigger than
    // SPI_MEM_ECC_ERR_INT_NUM. When SPI_FMEM_ECC_ERR_INT_EN is cleared and  SPI_SMEM_ECC_ERR_INT_EN is set, this bit is
    // triggered when the error times of SPI0/1 ECC read external RAM are equal or bigger than SPI_MEM_ECC_ERR_INT_NUM.
    // When SPI_FMEM_ECC_ERR_INT_EN and  SPI_SMEM_ECC_ERR_INT_EN are set, this bit is triggered when the total error
    // timesof SPI0/1 ECC read external RAM and flash are equal or bigger than SPI_MEM_ECC_ERR_INT_NUM. When
    // SPI_FMEM_ECC_ERR_INT_EN and  SPI_SMEM_ECC_ERR_INT_EN are cleared, this bit will not be triggered.
    uint32_t ecc_err_int_raw          : 1;
    // The raw bit for SPI_MEM_PMS_REJECT_INT interrupt. 1: Triggered when SPI1 accessis rejected. 0: Others.
    uint32_t pms_reject_int_raw       : 1;
    // The raw bit for SPI_MEM_AXI_RADDR_ERR_INT interrupt. 1: Triggered when AXI readaddress is invalid by compared to
    // MMU configuration. 0: Others.
    uint32_t axi_raddr_err_int_raw    : 1;
    // The raw bit for SPI_MEM_AXI_WR_FALSH_ERR_INT interrupt. 1: Triggered when AXI write flash request is received. 0:
    // Others.
    uint32_t axi_wr_flash_err_int_raw : 1;
    // The raw bit for SPI_MEM_AXI_WADDR_ERR_INT interrupt. 1: Triggered when AXI write address is invalid by compared
    // to MMU configuration. 0: Others.
    uint32_t axi_waddr_err_int_raw    : 1;
    // The raw bit for SPI_MEM_BROWN_OUT_INT interrupt. 1: Triggered condition is thatchip is loosing power and RTC
    // module sends out brown out close flash request toSPI1. After SPI1 sends out suspend command to flash, this
    // interrupt is triggered and MSPI returns to idle state. 0: Others.
...
typedef volatile struct {
    ...
    // Set this bit to mask the first or the last byte in SPI0 ECC DDR read mode, whenaccesses to flash.
    ...
    // 1: Do not need the input of SPI_DQS signal, SPI0 starts to receive data when spi0_slv_st is in SPI_MEM_DIN state.
    // It is used when there is no SPI_DQS signal orSPI_DQS signal is not stable. 0: SPI0 starts to store data at the
    // positive and negative edge of SPI_DQS.
    ...
    // Set this bit to enable the vary dummy function in SPI HyperBus mode, when SPI0 accesses flash or SPI1 accesses
    // flash or sram.
    uint32_t fmem_hyperbus_dummy_2x : 1;
    // Set this bit to invert SPI_DIFF when accesses to flash. .
    uint32_t fmem_clk_diff_inv      : 1;
    // Set this bit to enable octa_ram address out when accesses to flash, which meansADDR_OUT[31:0] =
    // {spi_usr_addr_value[25:4], 6'd0, spi_usr_addr_value[3:1], 1'b0}.
    uint32_t fmem_octa_ram_addr     : 1;
    // Set this bit to enable HyperRAM address out when accesses to flash, which meansADDR_OUT[31:0] =
...
typedef volatile struct {
    ...
    // Set this bit to mask the first or the last byte in SPI0 ECC DDR read mode, whenaccesses to external RAM.
    ...
    // Set this bit to enable octa_ram address out when accesses to external RAM, which means ADDR_OUT[31:0] =
    // {spi_usr_addr_value[25:4], 6'd0, spi_usr_addr_value[3:1], 1'b0}.
    uint32_t smem_octa_ram_addr     : 1;
    // Set this bit to enable HyperRAM address out when accesses to external RAM, which means ADDR_OUT[31:0] =
    // {spi_usr_addr_value[19:4], 13'd0, spi_usr_addr_value[3:1]}.
    uint32_t smem_hyperbus_ca       : 1;
    uint32_t                        : 1;
} spimem_spi_smem_ddr_t;

typedef volatile struct {
    // 1: SPI1 flash ACE section $n read accessible. 0: Not allowed.
    uint32_t fmem_pms_rd_attr : 1;
    // 1: SPI1 flash ACE section $n write accessible. 0: Not allowed.
    uint32_t fmem_pms_wr_attr : 1;
    // SPI1 flash ACE section $n ECC mode, 1: enable ECC mode. 0: Disable it. The flash ACE section $n is configured by
    // registers SPI_FMEM_PMS$n_ADDR_REG and SPI_FMEM_PMS$n_SIZE_REG.
    uint32_t fmem_pms_ecc     : 1;
    uint32_t                  : 29;
} spimem_spi_fmem_pms_attr_t;

typedef volatile struct {
    // SPI1 flash ACE section $n start address value
    uint32_t fmem_pms_addr_s : 26;
    uint32_t                 : 6;
} spimem_spi_fmem_pms_addr_t;

typedef volatile struct {
    // SPI1 flash ACE section $n address region is (SPI_FMEM_PMS$n_ADDR_S, SPI_FMEM_PMS$n_ADDR_S + SPI_FMEM_PMS$n_SIZE)
    uint32_t fmem_pms_size : 14;
    uint32_t               : 18;
} spimem_spi_fmem_pms_size_t;

typedef volatile struct {
    // 1: SPI1 external RAM ACE section $n read accessible. 0: Not allowed.
    uint32_t smem_pms_rd_attr : 1;
    // 1: SPI1 external RAM ACE section $n write accessible. 0: Not allowed.
    uint32_t smem_pms_wr_attr : 1;
    // SPI1 external RAM ACE section $n ECC mode, 1: enable ECC mode. 0: Disable it. The external RAM ACE section $n is
    // configured by registers SPI_SMEM_PMS$n_ADDR_REG and SPI_SMEM_PMS$n_SIZE_REG.
    uint32_t smem_pms_ecc     : 1;
    uint32_t                  : 29;
} spimem_spi_smem_pms_attr_t;

typedef volatile struct {
    // SPI1 external RAM ACE section $n start address value
    uint32_t smem_pms_addr_s : 26;
    uint32_t                 : 6;
} spimem_spi_smem_pms_addr_t;

typedef volatile struct {
    // SPI1 external RAM ACE section $n address region is (SPI_SMEM_PMS$n_ADDR_S, SPI_SMEM_PMS$n_ADDR_S +
    // SPI_SMEM_PMS$n_SIZE)
    uint32_t smem_pms_size : 14;
    uint32_t               : 18;
} spimem_spi_smem_pms_size_t;

typedef volatile struct {
    // This bits show the first SPI1 access error address. It is cleared by when  SPI_MEM_PMS_REJECT_INT_CLR bit is set.
    uint32_t reject_addr   : 26;
    // Set this bit to enable SPI0/1 transfer permission control function.
    uint32_t pm_en         : 1;
    uint32_t               : 1;
    // 1: SPI1 write access error. 0: No write access error. It is cleared by when  SPI_MEM_PMS_REJECT_INT_CLR bit is
    // set.
    uint32_t pms_ld        : 1;
    // 1: SPI1 read access error. 0: No read access error. It is cleared by when  SPI_MEM_PMS_REJECT_INT_CLR bit is set.
    uint32_t pms_st        : 1;
    // 1: SPI1 access is rejected because of address miss. 0: No address miss error. It is cleared by when
    // SPI_MEM_PMS_REJECT_INT_CLR bit is set.
    uint32_t pms_multi_hit : 1;
    // 1: SPI1 access is rejected because of address multi-hit. 0: No address multi-hit error. It is cleared by when
    // SPI_MEM_PMS_REJECT_INT_CLR bit is set.
    uint32_t pms_ivd       : 1;
} spimem_pms_reject_t;
...
typedef volatile struct {
    ...
    // Set this bit to enable MSPI ECC address conversion, no matter MSPI accesses to the ECC region or non-ECC region
    // of flash. If there is no ECC region in flash, this bit should be 0. Otherwise, this bit should be 1.
    ...
    uint32_t                            : 2;
    // 1: The error information in SPI_MEM_ECC_ERR_BITS and SPI_MEM_ECC_ERR_ADDR is updated when there is an ECC error.
    // 0: SPI_MEM_ECC_ERR_BITS and SPI_MEM_ECC_ERR_ADDR record the first ECC error information.
    uint32_t ecc_continue_record_err_en : 1;
    // Records the first ECC error bit number in the 16 bytes(From 0~127, corresponding to byte 0 bit 0 to byte 15 bit
    // 7)
...
} spimem_ecc_err_addr_t;

typedef volatile struct {
    // This bits show the first AXI write/read invalid error or AXI write flash error address. It is cleared by when
    // SPI_MEM_AXI_WADDR_ERR_INT_CLR, SPI_MEM_AXI_WR_FLASH_ERR_IN_CLR or SPI_MEM_AXI_RADDR_ERR_IN_CLR bit is set.
    uint32_t axi_err_addr              : 26;
    // The empty status of all AFIFO and SYNC_FIFO in MSPI module. 1: All AXI transfers and SPI0 transfers are done. 0:
    // Others.
    ...
    // This bit is set when WADDR_AFIFO, WBLEN_AFIFO, WDATA_AFIFO, AXI_RADDR_CTL_AFIFOand RDATA_AFIFO are empty and
    // spi0_mst_st is IDLE.
    uint32_t all_axi_trans_afifo_empty : 1;
} spimem_axi_err_addr_t;
...
typedef volatile struct {
    ...
    // Set this bit to enable MSPI ECC address conversion, no matter MSPI accesses to the ECC region or non-ECC region
    // of external RAM. If there is no ECC region in external RAM, this bit should be 0. Otherwise, this bit should
    // be 1.
    uint32_t smem_ecc_addr_en    : 1;
    uint32_t                     : 11;
} spimem_spi_smem_ecc_ctrl_t;
...
typedef volatile struct {
    ...
    uint32_t                      : 25;
} spimem_timing_cali_t;

typedef volatile struct {
    // the input signals are delayed by system clock cycles, 0: input without delayed,1: input with the posedge of
    // clk_apb,2 input with the negedge of clk_apb,  3: input with the posedge of clk_160, 4 input with the negedge of
    // clk_160, 5: input with the spi_clk high edge,  6: input with the spi_clk low edge
    ...
    // the input signals are delayed by system clock cycles, 0: input without delayed,1: input with the posedge of
    // clk_apb,2 input with the negedge of clk_apb, 3: input with the spi_clk
...
typedef volatile struct {
    // the output signals are delayed by system clock cycles, 0: output without delayed, 1: output with the posedge of
    // clk_apb,2 output with the negedge of clk_apb, 3: output with the posedge of clk_160,4 output with the negedge of
    // clk_160,5: output with the spi_clk high edge ,6: output with the spi_clk low edge
    ...
    // the output signals are delayed by system clock cycles, 0: output without delayed, 1: output with the posedge of
    // clk_apb,2 output with the negedge of clk_apb, 3: output with the spi_clk
...
typedef volatile struct {
    ...
    // For SPI0 and SPI1, spi cs signal is delayed to inactive by spi clock this bits are combined with spi_mem_cs_hold
    // bit.
    uint32_t smem_cs_hold_time         : 5;
    // SPI_SMEM_CS_HOLD_TIME + SPI_SMEM_ECC_CS_HOLD_TIME is the SPI0 and SPI1 CS hold cycles in ECC mode when accessed
    // external RAM.
    ...
    uint32_t                           : 8;
    // These bits are used to set the minimum CS high time tSHSL between SPI burst transfer when accesses to external
    // RAM. tSHSL is (SPI_SMEM_CS_HOLD_DELAY[5:0] + 1) MSPI core clock cycles.
    uint32_t smem_cs_hold_delay        : 6;
    // Set this bit to enable SPI0 split one AXI accesses EXT_RAM transfer into two SPI transfers when one transfer will
    // cross flash/EXT_RAM page corner, valid no matter whether there is an ECC region or not.
    uint32_t smem_split_trans_en       : 1;
} spimem_spi_smem_ac_t;
...
} spimem_mmu_power_ctrl_t;

// SPI memory map controller peripheral.
typedef struct {
    spimem_cmd_t cmd;
    // In user mode, it is the memory address. other then the bit0-bit23 is the memoryaddress, the bit24-bit31 are the
    // byte length of a transfer.
    uint32_t volatile addr;
    spimem_ctrl_t  ctrl;
    spimem_ctrl1_t ctrl1;
    spimem_ctrl2_t ctrl2;
    spimem_clock_t clock;
    spimem_user_t  user;
    spimem_user1_t user1;
    spimem_user2_t user2;
    volatile struct {
        // The length in bits of write-data. The register value shall be (bit_num-1).
        uint32_t mosi_dlen_usr_mosi_dbitlen : 10;
        uint32_t                            : 22;
    };
    volatile struct {
        // The length in bits of  read-data. The register value shall be (bit_num-1).
        uint32_t miso_dlen_usr_miso_dbitlen : 10;
        uint32_t                            : 22;
    };
    spimem_rd_status_t rd_status;
    // Padding.
    uint32_t volatile _padding0[1];
    spimem_misc_t misc;
    // For SPI1, the value of crc32.
    uint32_t volatile tx_crc;
    spimem_cache_fctrl_t  cache_fctrl;
    spimem_cache_sctrl_t  cache_sctrl;
    spimem_sram_cmd_t     sram_cmd;
    spimem_sram_drd_cmd_t sram_drd_cmd;
    spimem_sram_dwr_cmd_t sram_dwr_cmd;
    spimem_sram_clk_t     sram_clk;
    volatile struct {
        uint32_t                     : 7;
        // The lock delay time of SPI0/1 arbiter by spi0_slv_st, after PER is sent by SPI1.
        uint32_t fsm_lock_delay_time : 5;
        uint32_t                     : 20;
    };
    // data buffer
    uint32_t volatile w[16];
    spimem_flash_waiti_ctrl_t flash_waiti_ctrl;
    spimem_flash_sus_ctrl_t   flash_sus_ctrl;
    spimem_flash_sus_cmd_t    flash_sus_cmd;
    spimem_sus_status_t       sus_status;
    // Padding.
    uint32_t volatile _padding1[6];
    spimem_int_ena_t int_ena;
    spimem_int_clr_t int_clr;
    spimem_int_raw_t int_raw;
    spimem_int_st_t  int_st;
    // Padding.
    uint32_t volatile _padding2[1];
    spimem_ddr_t          ddr;
    spimem_spi_smem_ddr_t spi_smem_ddr;
    // Padding.
    uint32_t volatile _padding3[9];
    spimem_spi_fmem_pms_attr_t spi_fmem_pms_attr[4];
    spimem_spi_fmem_pms_addr_t spi_fmem_pms_addr[4];
    spimem_spi_fmem_pms_size_t spi_fmem_pms_size[4];
    spimem_spi_smem_pms_attr_t spi_smem_pms_attr[4];
    spimem_spi_smem_pms_addr_t spi_smem_pms_addr[4];
    spimem_spi_smem_pms_size_t spi_smem_pms_size[4];
    // Padding.
    uint32_t volatile _padding4[1];
    spimem_pms_reject_t        pms_reject;
    spimem_ecc_ctrl_t          ecc_ctrl;
    spimem_ecc_err_addr_t      ecc_err_addr;
    spimem_axi_err_addr_t      axi_err_addr;
    spimem_spi_smem_ecc_ctrl_t spi_smem_ecc_ctrl;
    // Padding.
    uint32_t volatile _padding5[2];
    spimem_timing_cali_t          timing_cali;
    spimem_din_mode_t             din_mode;
    spimem_din_num_t              din_num;
    spimem_dout_mode_t            dout_mode;
    spimem_spi_smem_timing_cali_t spi_smem_timing_cali;
    spimem_spi_smem_din_mode_t    spi_smem_din_mode;
    spimem_spi_smem_din_num_t     spi_smem_din_num;
    spimem_spi_smem_dout_mode_t   spi_smem_dout_mode;
    spimem_spi_smem_ac_t          spi_smem_ac;
    // Padding.
    uint32_t volatile _padding6[23];
    volatile struct {
        // Register clock gate enable signal. 1: Enable. 0: Disable.
        uint32_t clock_gate_clk_en : 1;
        uint32_t                   : 31;
    };
    // Padding.
    uint32_t volatile _padding7[94];
    // MSPI-MMU item content
    uint32_t volatile mmu_item_content;
    // MSPI-MMU item index
    uint32_t volatile mmu_item_index;
    spimem_mmu_power_ctrl_t mmu_power_ctrl;
    // Padding.
    uint32_t volatile _padding8[26];
    // ECO high register
    uint32_t volatile registerrnd_eco_high;
    // ECO low register
    uint32_t volatile registerrnd_eco_low;
    // Padding.
    uint32_t volatile _padding9[1];
